...
inputDir = parentDir + "\\processed_images\\registered_images" + File.separator;
maskDir = parentDir + "\\processed_images\\grain_mask" + File.separator;

homeDir = getDirectory("home");
desktopDir =  File.separator;
backSubDir = homeDir + "Desktop" + "\\background_subtracted" + File.separator;
ThreshDir = homeDir + "Desktop" + "\\thresholded_images" + File.separator;
File.makeDirectory(backSubDir);
File.makeDirectory(ThreshDir);
...
for (i = 0; i < lengthOf(filelist); i++) {
    if (endsWith(filelist[i], "inverted_mask.tif")) { 
...
for (i = 1; i < nFiles; i++) {
    ...
    if (i == 1) {
    	saveAs("Tiff", backSubDir + "Image_00.tif");    	
    }
    saveAs("Tiff", backSubDir + "Image_" + num + ".tif");
...
paramFile = backSubDir + "subtraction_parameters.txt";
